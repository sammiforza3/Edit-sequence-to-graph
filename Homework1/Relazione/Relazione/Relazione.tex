...
\begin{enumerate}
    ...
    \begin{itemize}
        \item M[0, s0] = 0
        \item Colonna iniziale (pattern avanza, grafo fermo): M[i, s0] = i $\times$ g
        \item Riga iniziale (grafo avanza, pattern fermo): M[0, v] = $\min_{u \in Pred(v)} \{ M[0, u] \} + g$ seguendo l'ordinamento topologico.
    \end{itemize}
    ...
    \item Backtracking: Il costo della minima distanza genomica si troverà nella cella M[k, t]. Partendo da questa cella e seguendo i puntatori a ritroso fino a s0, si ricostruisce l'elenco dei vertici del cammino ottimo.
\end{enumerate}
...
cella $M[m,n]$ dove m è il numero di righe e n il numero di colonne. 
